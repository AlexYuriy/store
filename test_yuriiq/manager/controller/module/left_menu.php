...
class ControllerModuleLeftMenu extends Controller {
	...
	public function index() {   
		...
		$this->load->language('module/left_menu');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('left_menu', $this->request->post);		
		...
		//
		$config_data = array(
				'left_menu_example' //this becomes available in our view by the foreach loop just below.
		);
		...
		//SET UP BREADCRUMB TRAIL. YOU WILL NOT NEED TO MODIFY THIS UNLESS YOU CHANGE YOUR MODULE NAME.
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/left_menu', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/left_menu', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['left_menu_module'])) {
			$this->data['modules'] = $this->request->post['left_menu_module'];
		} elseif ($this->config->get('left_menu_module')) { 
			$this->data['modules'] = $this->config->get('left_menu_module');
		}		
		...
		$this->template = 'module/left_menu.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/left_menu')) {
