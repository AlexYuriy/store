...
class ControllerLocalisationCity extends Controller {
	...
	protected function getList() {
		...
		$results = $this->model_localisation_city->getCity($data);
		...
		$this->data['sort_name'] = $this->url->link('localisation/city', 'token=' . $this->session->data['token'] . '&sort=z.name' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if (isset($this->request->get['city_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$zone_info = $this->model_localisation_zone->getZone($this->request->get['city_id']);
		}
