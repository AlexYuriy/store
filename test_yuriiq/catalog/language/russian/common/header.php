<?php

// text
$_['text_news']	= 'Новости';
$_['text_account']	= 'Личный кабинет';
$_['text_logged']	= '<i class="fa fa-user"></i> %s';
$_['text_welcome']	= '<a href="%s"><i class="fa fa-user"></i> Войти</a>  <a href="%s"><i class="fa fa-pencil"></i> Зарегистрироваться</a>';
$_['text_special']	= 'Акции';
$_['text_page']	= 'страница';
$_['text_search']	= 'Поиск';
$_['text_wishlist']	= 'Закладки (%s)';
$_['text_brands']	= 'Производители';
$_['text_logout']	= 'Выйти';
$_['text_q_and_a']	= 'Вопросы и ответы';
$_['text_home']	= 'Главная';
$_['text_shopping_cart']	= 'Корзина покупок';
$_['text_latest']	= 'Новинки';
$_['text_blog']	= 'Блог';
$_['text_checkout']	= 'Оформление заказа';
$_['text_carts']	= 'Сохранённые корзины';
$_['text_categories']	= 'Категории товаров';
?>